...
class GroundEnv(gym.Env):
    ...
    def __init__(self, tasks={}, map='b', render_mode='none', label='', actiontype='serial', statetype='image3c'):
        ...
        self.dsm = mapgdal.GetRasterBand(1).ReadAsArray()
        self.mapmask = buildlabelgdal.GetRasterBand(1).ReadAsArray()
        self.tmask = treelabelgdal.GetRasterBand(1).ReadAsArray()
        self.map = np.stack((self.mapmask, self.dsm), axis=2)
        ...
        self.basepos = tasks['basepos'][:POS_D] #basement's position
        self.tasknum = tasks['tasknum']  #tasks'number
        self.maxstep = tasks['maxstep']  #max step in all task
        self.cliposs_begin = tasks['cliposs'][:,:POS_D] #current client's position
        self.paths = tasks['paths']
    ...
    def step(self, action):
        ...
        if self.count < self.maxstep: # 计数是否到了最大一步
            #这个cli是否更新完了
            cliupdate = np.empty(shape=(0,POS_D))
            for i in range(self.tasknum):
                if self.count < self.paths[i].shape[0]:
                    cliupdate = np.append(cliupdate, self.paths[i][self.count][:POS_D].reshape(1, POS_D), axis=0)
                else:
                    cliupdate = np.append(cliupdate, np.zeros(shape=(1,POS_D)), axis=0)
            self.cliposs = np.clip(self.cliposs + cliupdate * CLIENT_SPEED, 0, self.size - 1)
            self.count += 1
        ...
        r1 = np.sum(cli2base < self.speed_threshold) / len(cli2base)
        r1 = 1 #test
        #4.奖励怎么给
