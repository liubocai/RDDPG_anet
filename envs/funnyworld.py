...
import pygame
import cv2
#1_episodes2000_steps99_var3.The variables representing the coordinates are represented by nd.array
...
class GroundEnv(gym.Env):
    ...
    def __init__(self, tasks=[], map='b224', render_mode="none"):
        self.render_mode = render_mode
        #1_episodes2000_steps99_var3.Environment
        ...
        self.speed_threshold = 35
        self.vid = cv2.VideoWriter(PROJECT_PATH + '/video/fn1.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 24, self.size)
        ...
        # self.observation_space = spaces.Box(low=np.array([0,0]), high = np.array([i-1 for i in size]), shape=(2,), dtype=int)

        #4.visiable
        self.window = None
        self.window_size_x = self.size[1]
        self.window_size_y = self.size[0] #int(self.window_size_x * self.size[1_episodes2000_steps99_var3] / self.size[0])
        self.clock = None
    ...
    def updateRadioPosByMeans(self):
        all = np.row_stack([self.cliposs, self.basepos.reshape((1,2))])
        self._radio_position = np.clip(np.mean(all, axis=0), 0, self.size-1)
    ...
    def step(self, action):
        #1_episodes2000_steps99_var3.update radio's pos
        # direction = self._action_to_direction[action] * self.radiospeed
        # self._radio_position = np.clip(
        #     self._radio_position + direction, 0, [i-1_episodes2000_steps99_var3 for i in self.size]
        # )
        self._radio_position = np.clip(
            self._radio_position + action * self.radiospeed, 0, self.size-1
        )
        # self.updateRadioPosByMeans()
    ...
    def _render_frame(self):
        ...
        if self.render_mode == "human":
            ...
            if self.vid.isOpened():
                ...
                self.vid = cv2.VideoWriter(PROJECT_PATH + '/video/fn1.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 6, self.size)
    def close(self):
